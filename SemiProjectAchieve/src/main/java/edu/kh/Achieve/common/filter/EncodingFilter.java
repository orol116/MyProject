 ...
 * 
 * 	[요청 흐름]
 * 	클라이언트가 요청하면 -> HttpServletRequest, HttpServletResponse 생성 -> !!!필터!!! -> 요청 처리 Servlet으로 이동
 * 	
 *  - 여러 필터를 만들어 연쇄적으로 연결하여 수행할 수 있음 (FilterChain)
 *
 */

/*
 * @WebFilter("url 패턴") : 해당 클래스를 필터 클래스로 등록
	- url 패턴에 일치하는 요청이 있을 경우 그 요청을 필터링함 
