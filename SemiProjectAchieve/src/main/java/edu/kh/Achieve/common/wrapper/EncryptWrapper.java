package edu.kh.Achieve.common.wrapper;
...
//	생성자가 작성되어 있으면 컴파일러가 기본 생성자를 자동으로 추가하지 않음
...
//		super.getParameter(name) : 기존 getParameter() 메서드
		...
		switch(name) {
		case "inputPw" : // 회원가입할 때 입력한 비밀번호 (/WEB-INF/view/member/signUp.jsp)
		case "memberPw" : // 로그인할 때 입력한 비밀번호 (/index.jsp), 회원 탈퇴할 때 입력한 비밀번호(/WEB-INF/view/member/myPage-secession.jsp)
			
		case "currentPw" : // 비밀번호 변경 요청 시 입력한 비밀번호 (/WEB-INF/view/member/myPage-changePw.jsp)
		case "newPw" :
...
//		1. 해시 함수를 수행할 객체를 참조할 변수 선언
		...
		try {
//			2. SHA-512 방식의 해시 함수를 수행할 수 있는 객체를 얻어 옴
...
//			5. 암호화된 비밀번호를 encryptPw에 대입 -> byte[]을 String으로 변환해야 함
//			Base64 : byte코드를 문자열로 변환하는 객
